...
같은 네트워크의 PC에서 Raspberry Pi GUI에 접속하려면 GUI 서버를 모든 네트워크 인터페이스에 바인딩합니다.

```bash
./scripts/run.sh gui --config config.toml --host 0.0.0.0 --port 8080
```

Raspberry Pi의 IP 주소를 확인합니다.

```bash
hostname -I
```

예를 들어 IP가 `192.168.0.25`라면 PC 브라우저에서 아래 주소로 접속합니다.

```text
http://192.168.0.25:8080
```

방화벽을 사용하는 경우 8080 포트를 허용합니다.

```bash
sudo ufw allow 8080/tcp
```

`--host 0.0.0.0`은 같은 네트워크의 다른 기기도 접속할 수 있게 여는 설정입니다. 외부 인터넷에 직접 노출하지 말고 내부망에서만 사용하세요.
